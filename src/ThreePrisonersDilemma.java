...
public class ThreePrisonersDilemma {
	...
	class Teng_YeeJing_Player extends Player {
		private int punishCounter = 0;

		int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {
			// The tournament just start, be nice
			if (n == 0) 
				return 0;

			// Definitely last round, defect to earn extra payoff
			if (n == 109) 
				return 1;
			...
			// Start punishment if any player defects
	...
	class Wong_YitJian_Player extends Player{
		int selectAction(int n, int[] myHistory, int[] oppHistory1, int[] oppHistory2) {			
			//Stores only the previous 5 actions, because it is the most relevant
			int[] prevHistory1 = new int[5];
			int[] prevHistory2 = new int[5];
			
			//Stores the probability of the action with recent history has higher probability
			double[] probAction1 = new double[] {0.1, 0.1, 0.2, 0.2, 0.4};
			double[] probAction2 = new double[] {0.1, 0.1, 0.2, 0.2, 0.4};
			...
			//Always be nice in the first round
			if(n==0) return 0;
			
			
			//Use T4T strategy for the first five rounds
			if(n >= 1 && n < 5) {		
				if (Math.random() < 0.5)
					return oppHistory1[n-1];
				else
			...
			//Start after having history of previous 5 actions
			if(n >= 5) {
				for(int i = n-5; i <= n-1; i++) {
					int j = 0;
					//Assign the 5 most recent action to the dedicated array
					prevHistory1[j] = oppHistory1[i];
					prevHistory2[j] = oppHistory2[i];
					
					//Count the probability of action by multiplying the previous action to
					//its corresponding probability as set before and sum the each probability
					//up.
					probability1 += (prevHistory1[j]*probAction1[j]);
					probability2 += (prevHistory2[j]*probAction2[j]);
					j++;
					
				}	
			}
			//If the probability is higher than threshold, 
			if((probability1 >THRESHOLD || probability2 > THRESHOLD)) 
				return 1;
			else
				return 0;
		}
...
//	This is a helper function needed by scoresOfMatch.
	...
	Player makePlayer(int which) {
		...
		case 6: return new Wong_YitJian_Player();
		}
	...
	void runTournament() {
		...
		for (int i=0; i<numPlayers; i++) for (int j=i; j<numPlayers; j++) for (int k=j; k<numPlayers; k++) {
			...
			int rounds = 100; // Between 90 and 110 rounds
